...
public static class IconHelper
{
    ...
    public static string? ExtractAppIconToAppData(string filePath, int targetSize = 64, string aumid = "")
    {
        try
        {
            // Resolve .lnk to target so the extracted icon doesn't carry the shortcut arrow overlay
            if (!string.IsNullOrEmpty(filePath) && filePath.EndsWith(".lnk", StringComparison.OrdinalIgnoreCase))
            {
    ...
    /// <summary>Resolve a .lnk shortcut to its target path.</summary>
